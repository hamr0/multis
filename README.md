...
**Latest (0.18.0):** **M4 — native litectx memory.** Durable memory is rebuilt on litectx's promotion ladder, retiring the old two-stage capture pipeline and the homegrown `recent.json` window: every exchange is a short-lived **episode** (rolling ~90-day window, `memory.episode_window_days`), the ones you keep using are promoted **verbatim** to durable **facts**, and the conversation thread the bot replays is the ladder itself. `/remember` now **supersedes** a restated fact in place — say your deadline moved and it updates the existing note (and tells you what it replaced) instead of piling up a contradiction — tenant-fenced, fail-toward-keep. Semantic recall (meaning, not just keywords) is on by default. Consumes litectx **0.21→0.25** (per-tenant isolation, tenant-scoped `forget`, time-ordered recency + count, semantic-recall fence, supersede-by-key, configurable episode window), each validated against the published artifact; the homegrown memory store is now **zero**. Live-verified on Beeper (capture→recall across restart, `/forget` ceremony, supersession). **Next:** M5 context-engineering and `npm install -g` packaging. Full roadmap and per-POC status: **[PRD](docs/01-product/prd.md)** (migration tracker in Appendix A).
