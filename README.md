...
**Latest (0.17.7):** M10 unified owner-ask gate merged to `main` — one dispatcher behind every prompt that needs your PIN or a pick, which closes the "stuck on delete" replay bug (a destructive request you've answered no longer repeats) and gives PIN/picker/wizard flows one consistent behaviour. Live-verified on Beeper. Built on M9's intent-first governed core (0.17.1) and the full LIVE‡ security gate. **Next:** the native litectx memory model (M4 promotion ladder) and `npm install -g` packaging. Full roadmap and per-POC status: **[PRD](docs/01-product/prd.md)** (migration tracker in Appendix A).
