...
**Latest (0.17.8):** **M11 foundation refresh** — the agent loop and governance run on current bare-agent 0.19 + bareguard 0.9 (the natural-language PIN ceremony now halts the turn directly, no workaround), with a fail-closed cost cap for unpriceable rounds. Plain-text/CSV files now index and are searchable, and vague "what did I say?" questions fall back to your recent notes (litectx 0.19/0.20 consumed). A "Locked out" message no longer arrives twice; the temporary Beeper-timeout instrumentation is removed (cause traced to harmless suspend/startup). Live-verified on Beeper (destructive ceremony A–E + no poll-loop stalls). Built on M10's unified owner-ask gate and M9's intent-first governed core. **Next:** the native litectx memory model (M4 promotion ladder) and `npm install -g` packaging. Full roadmap and per-POC status: **[PRD](docs/01-product/prd.md)** (migration tracker in Appendix A).
