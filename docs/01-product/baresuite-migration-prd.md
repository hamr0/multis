...
> **Note (2026-06-17):** the obedient-bot-first dispatch rewrite (§8 register) landed the prompt/scope/command-detection/halt-UX core and **defers persona/constitution to M4/litectx**. The remaining router pending-state-machine de-tangle + destructive→PIN are the natural M6/M8 work; the named-assistant `active` mode below still stands.
