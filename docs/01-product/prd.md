...
### M4 — litectx memory *(dep: M3)* — replaces `src/memory/*` with the litectx ladder — 🔶 **LADDER DONE (incr 1+2; litectx 0.22.0; suite green, audit 0). BLOCKED on `memory-api` §R3 recency before completion + the 0.18.0 cut — multis will NOT ship the `recent.json` workaround (decision 2026-06-27).**
