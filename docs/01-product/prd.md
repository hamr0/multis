...
### M4 — litectx memory *(dep: M3)* — slims `src/memory/*` onto the litectx ladder — ✅ **INCREMENTS 1+2 DONE (litectx 0.22.0; suite green, audit 0). Two interim verbs await `recent-memory-by-scope` (§7); the ladder is fully live.**
- **Goal:** retire `memory.md`, the two-stage capture/condense pipeline, and ACT-R; rebuild durable memory on litectx kinds (episode→fact ladder). **`recent.json` (conversation window) is KEPT** — its removal is M5 (`assemble`/`recent-memory-by-scope`), not M4. The win is net deletion: the homegrown pipeline lives in litectx now.
...
- **KEEP (NOT M4's job — removal is M5):** `recent.json` (the cross-message conversation thread the agent loop needs — litectx has no time-ordered episode-recency verb yet; that is the open `recent-memory-by-scope` ask / M5 `assemble`/`summaryWindow`) + the raw daily logs (`appendToLog`, verbatim never-indexed forensic backup).
...
- **Increments:** (1) ✅ DONE — `src/context/index.js` ladder + forget methods (`rememberEpisode`/`rememberFact`/`recallMemory`/`promotionSweep`/`forgetMemory`) + integration cases; litectx `^0.21.0→^0.22.0` (isolation + forget asks DELIVERED+VALIDATED, 16/16 failable each); audit 0. (2) ✅ DONE — rewire + delete-dead in **one pass**: deleted `capture.js`, slimmed `manager.js` to the recent.json window + daily log, removed the doc-axis `rememberMemory`/`searchMemory` + their tests + the capture config knobs + the `simpleGenerate` import; episode-write at all 5 sites, `promotionSweep`, both prompt paths, both memory tools, the 3 app-verbs; updated 7 test stubs + the ceremony/memory tests to the ladder. **Suite 529/529, audit 0.** Scope shrank from 3 increments to 2 once it became clear deletion rides the rewire.
- **Two interim verbs await `recent-memory-by-scope`** (§7, filed 2026-06-27 — the recency read for `fact`/`episode`; `recentMemory` is doc-axis only): `recent.json` is KEPT as the conversation thread (agent-loop replay), and **`/memory` shows the recent conversation window** (not a durable-fact list — facts surface on demand via `recall_memory`). When that ask lands: `/memory` → `recentMemory({kind:['fact','episode']})`, agent history sources from episode-recency, and `recent.json` + the remaining `ChatMemoryManager` window code are deleted (homegrown store → zero; only raw daily logs remain by design).
...
- **Gate / Exit:** memory/recall tests rebuilt (+ a promotion-ladder test: episode recalled ≥threshold → appears as fact, neg control cold episode doesn't); §10.7 RAG re-run; smoke step 12 → e2e where possible.
...
| 2026-06-27 | M4 | litectx | **Time-ordered memory recency for `fact`/`episode`** (third memory-axis ask after isolation=recall + forget=delete): a per-tenant **recency** read — "newest N for this scope, no query" — tenant-fenced like recall/forget. `recentMemory` is doc-axis only; `recall` needs FTS terms; promotion/review are usage-ranked — none answer it. Powers `/memory` list-all + sourcing the agent message history from episode-recency (→ delete `recent.json`). **Full spec:** `litectx-asks/recent-memory-by-scope.md`. | ⛔ **OPEN (filed 2026-06-27).** Interim: `recent.json` kept as the conversation thread; `/memory` shows the conversation window; durable facts surface via `recall_memory`. **Non-blocking — the ladder is fully live without it.** |
