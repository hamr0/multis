...
- **R2 — `scope` on every row; the fence applies to BOTH `recall` AND `get`.** Scope is set **per-upload (at ingest time)** — a chat id — not per-instance (one multis process serves all chats, so `new LiteCtx({owner})` per chat is not viable). Default **null = unscoped/global**.
  - **`recall({scope})`** returns `scope ∪ null-global`, never any other scope — global kb stays visible from a customer chat; one customer never sees another's docs.
  - **`get(id, {scope})`** applies the **same fence to the direct handle**: a doc/blob tagged with a different scope returns `null` (a global/null-scope row stays visible to all). This is load-bearing, not scope creep — **fencing discovery alone does not deliver "one customer never sees another's,"** because ids are guessable (a derived id slugs the filename). Securing the *handle* is what completes R2. The optional-param shape keeps it backward-compatible: **bare `get(id)` is unchanged** (unfenced by id), so litectx's existing `owner`/`session` fetch-by-id model is untouched — the customer-tenant path opts into the stricter check because it has the stricter threat model (a reasoned asymmetry, not an inconsistency).
  - **multis's side of the bargain (multis-lane):** on every customer-/LLM-reachable path, multis **must always pass the requesting chat's `scope` to BOTH `recall` and `get`**, and **must never expose a bare `get(id)`** to a customer- or LLM-reachable surface. Namespacing ids per scope (`"<chat>:<doc>"`) rides along as cheap defense-in-depth. The *mechanism* is litectx's; *using it correctly on every call* is ours.
  - (Implementation — new column vs. extending `owner` — is litectx's call; the requirement is per-upload tagging + `scope∪global` on recall + the mismatched-scope→null fence on get.)
...
3. Docs under **two scopes** + one **global (null)** doc → a recall scoped to X returns **X's docs + the global doc, and nothing from the other scope**; an unscoped recall sees all. (validates R2 recall fence — global kb always visible; cross-customer fenced)
3b. `get(id, {scope:X})` for a doc tagged scope Y → **`null`**; for a scope-X doc or a global doc → returned; bare `get(id)` (no scope) → returned regardless. (validates R2 handle fence — a guessed/leaked id can't cross scopes when scope is passed)
