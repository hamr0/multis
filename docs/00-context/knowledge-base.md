...
## Configuration
Runtime config: ~/.multis/config.json. Secrets: .env. Governance: ~/.multis/auth/governance.json (read by `src/governance/gate.js`, mapped to a bareguard 0.9.0 Gate wired into bare-agent 0.19.0 via `wireGate`; verbatim args form, no field hoisting). Bareguard audit: ~/.multis/logs/gate.jsonl. App-event audit: ~/.multis/logs/audit.log. Shared budget: ~/.multis/run/budget.json.
