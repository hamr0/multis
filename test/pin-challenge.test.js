...
const { createPinChallenge, handleHumanReply, _clearAllPending } = require('../src/governance/human-channel');
...
// Deliver a reply once the challenge has registered its waiter.
async function deliverReply(senderId, text) {
  await new Promise(r => setTimeout(r, 10));
  return handleHumanReply(senderId, text);
}
...
describe('createPinChallenge (#5)', () => {
  ...
  it('no-ops (allows) when PIN is not configured', async () => {
    ...
    const challenge = createPinChallenge({ platformRegistry: new Map([['telegram', plat]]), pinManager: stubPin({ enabled: false }) });
  ...
  it('no-ops (allows) when the session is still fresh — no double prompt', async () => {
    const plat = mockPlatform();
    const challenge = createPinChallenge({ platformRegistry: new Map([['telegram', plat]]), pinManager: stubPin({ auth: false }) });
    assert.strictEqual(await challenge({ senderId: 'u', chatId: 'c', platform: 'telegram' }), true);
  ...
  it('prompts, accepts the correct PIN, resolves true', async () => {
    const plat = mockPlatform();
    const challenge = createPinChallenge({ platformRegistry: new Map([['telegram', plat]]), pinManager: stubPin(), timeoutMs: 1000 });
    const p = challenge({ senderId: 'u', chatId: 'c', platform: 'telegram' });
    assert.ok(await deliverReply('u', '1234'), 'reply was consumed by the waiter');
  ...
  it('times out to false when no reply arrives', async () => {
    const plat = mockPlatform();
    const challenge = createPinChallenge({ platformRegistry: new Map([['telegram', plat]]), pinManager: stubPin(), timeoutMs: 25 });
  ...
  it('fails closed (false) when the owner channel is unreachable', async () => {
    const challenge = createPinChallenge({ platformRegistry: new Map(), pinManager: stubPin() });
    assert.strictEqual(await challenge({ senderId: 'u', chatId: 'c', platform: 'telegram' }), false);
  });

  it('denies on lockout without waiting for input', async () => {
    const plat = mockPlatform();
    const challenge = createPinChallenge({ platformRegistry: new Map([['telegram', plat]]), pinManager: stubPin({ auth: 'locked' }) });
    assert.strictEqual(await challenge({ senderId: 'u', chatId: 'c', platform: 'telegram' }), false);
