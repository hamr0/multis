...
- **Bumped `bare-agent` `^0.19.0` → `^0.29.0`.** Every multis-consumed primitive (Loop, Retry, CircuitBreaker, HaltError, unitAssembler, Scheduler, Planner/runPlan, wireGate + defaultActionTranslator, the Anthropic/OpenAI/Ollama providers) verified against the installed artifact — no signature broke; the `halt:<rule>` error token (the ceremony park/lockout contract) is byte-identical. The bump also unsticks bare-agent's `bareguard` optional-peer range (`^0.9.0` → `>=0.9.0 <0.13.0`), which had been unsatisfied since multis moved to bareguard 0.12.0. Free pickups requiring no multis change: a truncated round's tool calls are refused, not executed with missing arguments (BA-4 closure); providers gracefully degrade when a model rejects a non-default temperature; Ollama's dropped `maxTokens` fixed.
- **Bounded loop terminations now reach chat honestly.** bare-agent ≥0.27 stopped laundering non-clean stops into clean empty answers: the Loop now reports `truncated:max_tokens` / `refusal` / `context_exceeded` (BA-6/BA-13), short-circuits a governance deny-spin at 3 consecutive denials (`denied:<tool>`, BA-11) and a byte-identical failing tool call at 3 repeats (`stuck:<tool>`, BA-12). multis's agent loop previously knew only `halt:*` tokens and would have thrown these raw at the chat as `LLM error: denied:exec`. It now maps each bounded exit to the model's preserved partial text (BA-5) plus an honest `(stopped early — …)` note, keeps tool names out of the reply (contacts can reach this path — the full token goes to the audit log as `loop_bounded`), and still throws on genuinely unexpected errors. Where a truncated or safety-refused round used to read as a silent empty answer and a deny-spin burned every tool round, both now end early and say so. Regression-tested red→green through the real router, including a live deny-spin through the wired bareguard gate.
- **Bumped `litectx` `^0.27.0` → `^0.30.0`.** Validated against the installed artifact with a DB written by 0.27.0: the index-format-stamp migration (0.29.0) opens the old DB cleanly, buffer-ingested doc bodies survive the new stale-pointer drift gate (`recall({body:true})` intact — the gate keys on content hash, which a buffer-ingested doc can't drift), facts/episodes and scope fences hold, `scoped().forget({id})` (M14) and `ingest → {chunks, mode}` unchanged, `assemble` (M5) still exported. 0.28.0's `WRITE_KINDS` export and 0.30.0's MCP warm-index/embedding-backfill are additive or MCP-only — inert for multis. No multis code change needed.
...
- The publish workflow stays pinned to `npm@11` (which builds native addons without the allowlist and, unlike npm 12.0.0, has a working `npm publish --provenance` — npm 12.0.0's provenance path `require`s a `sigstore` module it fails to bundle). The committed `allowScripts` allowlist is there for consumers on npm 12; CI moves to `npm@12 + npm ci --strict-allow-scripts` once npm fixes provenance publishing.
...
This is pure context-engineering, handled by litectx's `assemble` (0.25.0) wired through bare-agent's `Loop` (0.19.0) — **the same libraries multis already depends on**, no new dependency. It does **not** touch the documents or saved notes the assistant retrieves to answer a question — those are still the top few most-relevant. Full suite green, `npm audit` clean.
...
### Fixed — a wrong PIN is retry-able, not a dead end

Entering a wrong PIN used to cancel the action outright, so your next (correct) PIN had nothing to resume and fell through. Now a wrong PIN **re-parks** the same action and tells you how many attempts remain (the 3 you always had before lockout) — reply the correct PIN and it runs. A genuine lockout (3 strikes, or a reply during an active lockout) is still terminal.

### Fixed — internal: cleaner audit + lockout signalling

A parked PIN ceremony is no longer mis-logged as a `loop_error` in `audit.log` (it's a clean governance exit; the action's `govern` line is the real record). A PIN reply received *during* an active lockout window is now treated as terminal rather than silently re-parking.
