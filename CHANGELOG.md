...
- **`/remember <note>`** still saves a durable note instantly (top trust, no waiting) — and now, if your note **restates something it already knows** (a moved deadline, a corrected detail), it **updates that fact in place** instead of leaving two contradictory notes. Distinct facts are kept side by side; only a genuine same-subject update overwrites, and never across chats.
